...
export async function getTransactionById(
  ...
  try {
    ...
    const transaction = data as unknown as TransactionWithUser;

    // Generate signed URL if image exists
    if (transaction.image_url) {
      const BUCKET_NAME =
        process.env.SUPABASE_STORAGE_BUCKET || "bukti-transfer";
      let path = transaction.image_url;

      // Extract path if it's a full URL
      if (path.includes("/storage/v1/object/public/")) {
        const parts = path.split(`/public/${BUCKET_NAME}/`);
        if (parts.length > 1) {
          path = parts[1];
        }
      }
      ...
      if (signedData?.signedUrl) {
        transaction.image_url = signedData.signedUrl;
      }
    }
...
export async function getAllTransactionsForExport(): Promise<
